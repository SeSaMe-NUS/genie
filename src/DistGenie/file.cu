...
namespace distgenie
{
	void file::ReadData(GPUGenie_Config &config, ExtraConfig &extra_config, vector<vector<int> > &data, vector<inv_table*> &tables)
	{
		if (0 == g_mpi_rank)
	...
	void file::GenerateOutput(vector<Result> &results, GPUGenie_Config &config, ExtraConfig &extra_config)
	{
