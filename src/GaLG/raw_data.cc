...
string* GaLG::raw_data::meta(int index)
{
...
int GaLG::raw_data::meta(string attr)
{
  int attr_number;
  for(attr_number = 0; attr_number < m_size(); attr_number++)
    if(_meta[attr_number].compare(attr) == 0) break;
  if(attr_number == m_size())
    attr_number = -1;
  return attr_number;
}

vector<string>* GaLG::raw_data::row(int index)
{
...
vector<string>* GaLG::raw_data::col(string attr)
{
  int attr_number;
  for(attr_number = 0; attr_number < m_size(); attr_number++)
    if(_meta[attr_number].compare(attr) == 0) break;
...
vector<string>* GaLG::raw_data::col(int attr_index)
{
