        ...
        do
        {
            ...
            for (int decomprRun = 0; decomprRun < totalDecomprRuns; decomprRun++)
            {
                ...
                if (table.getCompression() == "copy"){
                    decompressPostingLists_listPerThread_JustCopyCodec
                    ...
                    decompressPostingLists_listPerThread_DeviceDeltaCodec
                                <<<DECOMPR_BLOCKS, // Always run a fixed number of decompression blocks
                                   DECOMPR_THREADS_PER_BLOCK>>>
                           (d_dims_p, dims.size(), dimsOffset, table.deviceCompressedInv(), d_uncompr_inv_p,
                            table.getUncompressedPostingListMaxLength());
                    ...
                    decompressPostingLists_listPerThread_DeviceBitPackingCodec
                                <<<DECOMPR_BLOCKS, // Always run a fixed number of decompression blocks
                                   DECOMPR_THREADS_PER_BLOCK>>>
                           (d_dims_p, dims.size(), dimsOffset, table.deviceCompressedInv(), d_uncompr_inv_p,
                            table.getUncompressedPostingListMaxLength());
                    ...
                    decompressPostingLists_listPerThread_d1_bp32
                                <<<DECOMPR_BLOCKS, // Always run a fixed number of decompression blocks
                                   DECOMPR_THREADS_PER_BLOCK>>>
                           (d_dims_p, dims.size(), dimsOffset, table.deviceCompressedInv(), d_uncompr_inv_p,
                            table.getUncompressedPostingListMaxLength());
                ...
                match_adaptiveThreshold_queryPerBlock_compressed<<<batch,MATCH_THREADS_PER_BLOCK>>>
                       (table.m_size(),
